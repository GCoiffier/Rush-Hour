...
def clear() : #réinitialise l'éditeur de niveau
...
def clear_paths() : #réinitialise uniquement les chemins liés aux IA
    ...
    for x in IA :
        x.point_list = [way_point(zone_dessin,x.pos[1],x.pos[0],x.id)]
        ...
        if 1<=i<=19 and 1<=j<=30: # on ne doit pas changer les murs extérieurs
            ...
            if self.mode == 'path_creation' :
                ...
                if n == 6 or n==7 or n==10 or n==11 : #on a affaire à une IA qui nécessite un path
                    ...
                    pt.bind(cwia.point_list[-1])
                    pt.plot()
                    cwia.point_list.append(pt)
                    paths_matrix[cwia.id] = cwia.point_list
...
class IA_rectangle() :
    ...
    def __init__(self,can,i,j,color) :
        ...
        self.id = -1
        self.selected = False
        self.point_list = [way_point(zone_dessin,i,j,self.id)] #liste d'objets de classe way_point formant le chemin suivi
    ...
    def highlight_rect(self) :
        ...
        if not self.selected :
            ...
            self.point_list[0] = way_point(zone_dessin,i,j,self.id)
        else :
...
def level_editor() :
    ...
    for color in color_dict:
        ...
        button = color_button(zone_outils,4, 25*(n+1), 4+25, 25*(n+2), color)
